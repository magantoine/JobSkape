...
class WordEmbeddingModel():
    ...
    def init_model(self):
        self.word_emb = "jjzha/jobbert-base-cased"
        self.word_emb_tokenizer = AutoTokenizer.from_pretrained(self.word_emb)
        self.word_emb_model = AutoModel.from_pretrained(self.word_emb)
    
    def get_model(self):
        if not self.init :
            self.init_model()
        return self.word_emb_model
    
    def get_tokenizer(self):
        if not self.init :
            self.init_model()
        return self.word_emb_tokenizer
...
def embedd_tax(tax, show_progress=False, key_to_embedd="name+definition"):
    ...
    if(show_progress):
        tax["embeddings"] = tax[key_to_embedd].progress_apply(
                    lambda st : 
                    WordEmbeddingModel.get_model()(**WordEmbeddingModel.get_tokenizer()(st, return_tensors="pt", max_length=512, padding=True, truncation=True))
